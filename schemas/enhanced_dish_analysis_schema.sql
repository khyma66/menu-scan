-- Enhanced Schema for AI-Powered Dish Analysis
...
-- Function to update enhanced dish metadata
CREATE OR REPLACE FUNCTION update_dish_enhanced_metadata()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    
    -- Auto-classify dietary tags based on ingredients
    IF NEW.is_vegetarian = TRUE THEN
        NEW.dietary_tags = ARRAY['vegetarian'] || NEW.dietary_tags;
    END IF;
    
    IF NEW.is_vegan = TRUE THEN
        NEW.dietary_tags = ARRAY['vegan'] || NEW.dietary_tags;
    END IF;
    
    IF NEW.is_non_veg = TRUE THEN
        NEW.dietary_tags = ARRAY['non-vegetarian'] || NEW.dietary_tags;
    END IF;
...
CREATE TRIGGER update_dish_enhanced_metadata_trigger
    BEFORE UPDATE ON public.dishes
    FOR EACH ROW
    EXECUTE FUNCTION update_dish_enhanced_metadata();
